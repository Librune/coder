...
const BookDetail = () => {
  ...
  return bookDetail==undefined?<></> :(
    <div className="p-4 pt-0 select-text">
      ...
      <div className="flex">
        ...
        <div className="ml-4 flex-1 flex flex-col justify-between items-start text-left font-normal text-sm break-all max-h-[120px]">
          ...
          <div className="text-xs text-neutral-500 font-normal">
            {bookDetail!.author}/{bookDetail!.status}/{bookDetail!.wordCount}字
          </div>
          {!!bookDetail!.latestChapter && (
            <div className="text-xs text-neutral-500 font-normal">
              {bookDetail!.latestChapter!.name}
            </div>
          )}
      ...
      {!!bookDetail!.extraDatas && (
        <div className="flex flex-row items-center mt-2">
          {bookDetail!.extraDatas.map((item: any, index: number) => (
